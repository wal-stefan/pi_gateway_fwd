...
int get_concentrator_time(struct timeval *concent_time, struct timeval unix_time) {
    ...
    if (concent_time == NULL) {
        MSG_DEBUG(DEBUG_ERROR | DEBUG_TIMERSYNC, "ERROR: %s invalid parameter\n", __FUNCTION__);
        return -1;
    }
...
void thread_timersync(void) {
    ...
    while (!exit_sig && !quit_sig) {
        ...
        /*MSG_DEBUG(DEBUG_TIMERSYNC, "INFO~ host/sx1301 time offset=(%lds:%ldµs) - drift=%ldµs\n",
            offset_unix_concent.tv_sec,
            offset_unix_concent.tv_usec,
            offset_drift.tv_sec * 1000000UL + offset_drift.tv_usec);
        MSG_DEBUG(DEBUG_TIMERSYNC, "INFO~ Enabling GPS mode for concentrator's counter.\n\n");*/
